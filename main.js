...
Game.prototype.handleInput = function () {
  ...
  */
    ...
    if (keys["e"]) cam.vY += (5 * cam.levelSpeed - cam.vY) / 30;
    else cam.vY -= (cam.vY - cam.levelSpeed) / 15;

    if (keys["f"]) player.hasShield = true;
...
Game.prototype.updatePlayer = function () {
    if (this.player.time <= game.grace) this.player.hasShield = true;
    else this.player.hasShield = false;
    ...
    if (!this.playerAlive) {
        ...
        if (this.player.isOutside()) {
            ...
            if (keys["space"]) {
                if (this.levelNum >= this.numLevels) this.start();
                else {
                    this.loadLevel(this.levelNum + 1);
